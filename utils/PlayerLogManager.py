...
class PlayerLogManager:
    ...
    def log_chat(player_mgr, msg, chat_type):
        if config.Server.Logging.log_player_chat:
            ...
            PlayerLogManager._write_to_log(f'{date} [CHAT] (GUID {player_mgr.guid}, NAME {player_mgr.get_name()}, MAP {player_mgr.map_}, POS {PlayerLogManager._get_location_string(player_mgr)}): [{log_type}] {player_mgr.get_name()}:{player_mgr.guid} : {msg}')
            return
    ...
    def log_channel(player_mgr, msg, channel_name):
        if config.Server.Logging.log_player_chat:
            date = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
            PlayerLogManager._write_to_log(f'{date} [CHAT] (GUID {player_mgr.guid}, NAME {player_mgr.get_name()}, MAP {player_mgr.map_}, POS {PlayerLogManager._get_location_string(player_mgr)}): [Channel: {channel_name}] {player_mgr.get_name()}:{player_mgr.guid} : {msg}')
            return
    ...
    def log_whisper(player_mgr, msg, target_player_mgr):
        if config.Server.Logging.log_player_chat:
            date = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
            PlayerLogManager._write_to_log(f'{date} [CHAT] (GUID {player_mgr.guid}, NAME {player_mgr.get_name()}, MAP {player_mgr.map_}, POS {PlayerLogManager._get_location_string(player_mgr)}): [Whisper] {player_mgr.get_name()}:{player_mgr.guid} -> {target_player_mgr.get_name()}:{target_player_mgr.guid} : {msg}\n')
            return
