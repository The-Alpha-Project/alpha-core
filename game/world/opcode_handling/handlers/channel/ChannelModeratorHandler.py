...
class ChannelModeratorHandler:
    ...
    def handle_add_mod(world_session, reader):
        ...
        if target_player_mgr:
            ChannelManager.add_mod(channel, player_mgr, target_player_mgr)
        else:
    ...
    def handle_remove_mod(world_session, reader):
        ...
        target_player_mgr = WorldSessionStateHandler.find_player_by_name(player_name)
        if target_player_mgr:
            ChannelManager.remove_mod(channel, player_mgr, target_player_mgr)
        else:
            ChatManager.send_system_message(world_session, f'No player named [{player_name}] is currently playing.')
    ...
    def handle_moderate(world_session, reader):
        ...
        ChannelManager.toggle_moderation(channel, player_mgr)
