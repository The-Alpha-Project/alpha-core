...
class SpeedCheatHandler(object):
    ...
    def handle(world_session, socket, reader):
        if not world_session.player_mgr.is_gm:
            ...
            if reader.opcode == OpCode.MSG_MOVE_SET_RUN_SPEED_CHEAT.value:
                world_session.player_mgr.change_speed()
            elif reader.opcode == OpCode.MSG_MOVE_SET_SWIM_SPEED_CHEAT.value:
                world_session.player_mgr.change_swim_speed()
            elif reader.opcode == OpCode.MSG_MOVE_SET_WALK_SPEED_CHEAT.value:
                world_session.player_mgr.change_walk_speed()
            elif reader.opcode == OpCode.MSG_MOVE_SET_TURN_RATE_CHEAT.value:
