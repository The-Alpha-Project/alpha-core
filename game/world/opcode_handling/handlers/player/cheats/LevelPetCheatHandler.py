...
class LevelPetCheatHandler(object):
    ...
    def handle(world_session, socket, reader: PacketReader) -> int:
        ...
        if not world_session.account_mgr.is_gm():
            Logger.anticheat(f'Player {player_mgr.get_name()} ({player_mgr.guid}) tried to modify their pets level.')
            return 0
        ...
        if len(reader.data) >= 4:  # Avoid empty packet level cheat packet.
            ...
            active_pet.set_level(new_level, True)
