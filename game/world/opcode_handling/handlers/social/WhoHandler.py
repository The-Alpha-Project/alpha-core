...
class WhoHandler:
    ...
    def handle(world_session, reader):
        # Avoid handling an empty who packet.
        if not HandlerValidator.validate_packet_length(reader, min_length=26):
            return 0
        # TODO: Search for guild and faction handling
        level_min, level_max = unpack('<2I', reader.data[:8])

        current_size = 8
        player_name, current_size = WhoHandler._read_cstring(reader.data, current_size)
        if player_name is None:
            return 0

        guild_name, current_size = WhoHandler._read_cstring(reader.data, current_size)
        if guild_name is None:
            return 0

        # Mask block is always 12 bytes after both c-strings.
        if not HandlerValidator.validate_packet_length(reader, min_length=current_size + 12):
            return 0
        ...
        for x in range(zone_count):
            if not HandlerValidator.validate_packet_length(reader, min_length=current_size + 4):
                return 0
        ...
        for x in range(user_strings_count):
            user_string, current_size = WhoHandler._read_cstring(reader.data, current_size)
        ...
        for session in WorldSessionStateHandler.get_world_sessions():
            if session.player_mgr and session.player_mgr.online:
                ...
                if player_mgr.level < level_min or player_mgr.level > level_max:
                    continue
                if player_name and player_name.lower() not in player_mgr.get_name().lower():
                    continue
