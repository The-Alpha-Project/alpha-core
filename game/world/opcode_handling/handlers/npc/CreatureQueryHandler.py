    ...
    def handle(world_session, socket, reader):
        if len(reader.data) >= 12:  # Avoid handling empty creature query packet.
            ...
            if guid > 0:
                ...
                if creature_mgr:
                    player_mgr.enqueue_packet(QueryUtils.query_details(creature_mgr=creature_mgr))
                else:  # Fallback just in case.
                    creature_template = WorldDatabaseManager.CreatureTemplateHolder.creature_get_by_entry(entry)
                    if creature_template:
                        player_mgr.enqueue_packet(QueryUtils.query_details(creature_template=creature_template))
        return 0
