...
class BinderActivateHandler(object):
    ...
    def handle(world_session, socket, reader):
        if len(reader.data) >= 8:  # Avoid handling empty binder activate packet.
            ...
            if not binder or not binder.is_within_interactable_distance(world_session.player_mgr):
                return 0

            if world_session.player_mgr.deathbind.deathbind_zone == world_session.player_mgr.zone:
