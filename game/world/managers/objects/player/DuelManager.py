...
class DuelManager(object):
    ...
    def request_duel(self, target, arbiter_entry):
        ...
        arbiter = self._create_arbiter(target, arbiter_entry)
        if arbiter:
    ...
    def handle_duel_canceled(self):
        self.dueling_with.duel_manager.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_CANCELED_INTERRUPTED, self.dueling_with)
        self.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_CANCELED_INTERRUPTED, self.dueling_with)
    ...
    def force_duel_retreat(self):
        if self.dueling_with:
            self.dueling_with.duel_manager.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_FINISHED,
                                                    self.dueling_with)
            self.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_FINISHED, self.dueling_with)
    ...
    def end_duel(self, duel_winner_flag, duel_complete_flag, winner):
        ...
        self.duel_state = DuelState.DUEL_STATE_FINISHED
        if duel_winner_flag == DuelWinner.DUEL_WINNER_KNOCKOUT and winner == self.owner:
    ...
    def boundary_check(self, elapsed):
        ...
        if self.elapsed >= 1:
            ...
            if dist >= DuelManager.BOUNDARY_RADIUS:
                if self.duel_status == DuelStatus.DUEL_STATUS_OUTOFBOUNDS:
                    self.out_bounds_timer -= self.elapsed  # seconds
                    if self.out_bounds_timer <= 0:
                        self.dueling_with.duel_manager.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_FINISHED, self.dueling_with)
                        self.end_duel(DuelWinner.DUEL_WINNER_RETREAT, DuelComplete.DUEL_FINISHED, self.dueling_with)
                else:  # Was in range and now is out of bounds.
                    self.duel_status = DuelStatus.DUEL_STATUS_OUTOFBOUNDS
                ...
                if self.duel_status == DuelStatus.DUEL_STATUS_OUTOFBOUNDS:  # Just got in range again, notify
                    self.duel_status = DuelStatus.DUEL_STATUS_INBOUNDS
    ...
    def update(self, elapsed):
        if self.arbiter and self.duel_state == DuelState.DUEL_STATE_STARTED:
    ...
    def _create_arbiter(self, target, arbiter_entry):
