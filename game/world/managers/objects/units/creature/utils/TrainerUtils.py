...
class TrainerUtils:
    ...
    def get_training_list_spell_status(spell: Spell, trainer_spell_template: TrainerTemplate, req_level: int,
        ...
        if not fulfills_skill or (preceded_spell and preceded_spell not in target_spells) or \
                trainer_spell_template.reqskill != 0 and (not player_mgr.skill_manager.has_skill(trainer_spell_template.reqskill) or \
                player_mgr.skill_manager.get_total_skill_value(trainer_spell_template.reqskill) < trainer_spell_template.reqskillvalue):
            return TrainerServices.TRAINER_SERVICE_UNAVAILABLE
