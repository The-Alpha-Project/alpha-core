...
class CreatureManager(UnitManager):
    ...
    def _perform_combat_movement(self):
        if self.combat_target:
            ...
            # Fixes issue #234
            # If target is dead > evade
            if self.combat_target.is_alive == False:
                self.evade()
                return

            # TODO: Temp, extremely basic evade / runback mechanic based ONLY on distance. Replace later with a proper one.
            # Now also counts 10s since last taken damage until evade.
            if self.location.distance(self.spawn_position) > 50 and self.time_since_damage_taken > 50:
                self.evade()
                return
    ...
    def update(self, now):
        if now > self.last_tick > 0:
            ...
            if self.is_alive and self.is_spawned:
                ...
                # Update time_since_damage_taken for evade
                if self.in_combat:
                    self.time_since_damage_taken = self.time_since_damage_taken + 1
            # Dead
